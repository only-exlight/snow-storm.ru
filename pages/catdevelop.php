...
$Page = $_GET['page'];
$query ="SELECT name, description, date, id FROM art WHERE category = 'develop'"; // текст запроса
...
<a class="curr" href="#catdevelop/1">Разработка</a>
...
if($result) {
    ...
    //тут надо что то сделать что  бы не отображать странницу если записей нет
    echo '<div class="pagenav">Страница:';
    for ($i = 1; $i < ($rows/5 + 1); $i++){
        echo '<a href="#catdevelop/',$i,'">',$i,'</a>';
    }
    echo '</div>';
}
mysqli_close($link);
